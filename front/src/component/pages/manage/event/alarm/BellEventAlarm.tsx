...
import alarm from '@/assets/alarm/emergency_bell.mp3';

const AUTO_STOP_MS = 8000;
const GAP_MS = 500;

const BellEventAlarm = () => {
  const { isSocketAlarm, setIsSocketAlarm } = useEventStore(
    useShallow((state) => ({
      isSocketAlarm:    state.isSocketAlarm,
      setIsSocketAlarm: state.actions.setIsSocketAlarm,
    })),
  );
  ...
  useEffect(() => {
    ...
    if (isTargetUser && isSocketAlarm) {
      (async () => {
        const ok = await startAlarmWithAutoStop();
        if (ok) setIsSocketAlarm(false);
      })();
    }
  }, [isSocketAlarm, userInfo]);

  return <div />;
};

export default BellEventAlarm;
