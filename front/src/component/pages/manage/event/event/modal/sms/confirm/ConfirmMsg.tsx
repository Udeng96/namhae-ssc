...
const ConfirmMsg = () => {
  const { sms } = useEventStore((state) => ({ sms: state.sms }));

  return (
    <StyledPanel>
      <StyledHead>SMS</StyledHead>
      <StyledBody>
        <StyledSectionTitle>전파문구</StyledSectionTitle>
        <StyledField>
          <StyledFieldLabel>제목</StyledFieldLabel>
          <StyledInput disabled value={sms.title} />
        </StyledField>
        <StyledField>
          <StyledFieldLabel>내용</StyledFieldLabel>
          <StyledTextarea disabled value={sms.content} />
        </StyledField>
      </StyledBody>
      <ConfirmTarget targetList={sms.selectedTargets} />
    </StyledPanel>
  );
};
