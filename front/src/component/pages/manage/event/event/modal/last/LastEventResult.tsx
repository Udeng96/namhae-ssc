...
import LastEventList from './list/LastEventList';
...
const LastEventResult = ({ onReload, onPageChange, isError }: Props) => {
  const isFetching = useIsFetching({ queryKey: ['events', 'last'] }) > 0;
  const lastTotalCnt = useEventStore((state) => state.lastTotalCnt);
  ...
  return (
    <div>
      <StyledResult>
        ...
        <LastEventList onReload={onReload} onPageChange={onPageChange} isError={isError} />
      </StyledResult>
    </div>
  );
};

export default LastEventResult;
