...
/**
 * 시설 전용 그리기 컴포넌트
 * - 구 Draw.tsx와 동일한 기능이지만 신 gisStore.facDrawType을 직접 구독
 * - selectNav 의존 제거 (이미 시설 MapContainer 안에 있으므로 불필요)
 * - 시설 페이지는 반경(CIRCLE) 기능 없음 (DISTANCE / EXTENT 만 처리)
 */
const FacDraw = () => {
  const map = useMap();
  const facDrawType = useGisStore((s) => s.facDrawType);
  ...
  // facDrawType 변경 → activeType 갱신 (selectNav 불필요)
  useEffect(() => {
    setActiveType(facDrawType);
  }, [facDrawType]);
