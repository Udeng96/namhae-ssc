...
const EventPoi = ({ area, top, left, hoverTop, hoverLeft }: EventPoiProps) => {
  const { hoverArea, eventResult, setHoverArea, setActiveArea } = useStatStore(
    useShallow((s) => ({
  ...
  const gisStats: GisStatType[] = eventResult?.gisStats ?? [];
  const allCnt: number = eventResult?.allCnt ?? 0;
  ...
  const bgImage = isHovered
    ? STAT_POI_COLOR_HOVER_GRADE[grade]
    : STAT_POI_COLOR_GRADE[grade];
...
const StyledEventPoi = styled.button<StyledPoiProps>`
  position: absolute;
  top:43px;
  left: 31px;
  width: 68px;
