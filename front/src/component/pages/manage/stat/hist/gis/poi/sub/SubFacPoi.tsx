...
import { STAT_POI_FAC_COLOR_GRADE } from '@/component/constants/statConst';
import { ZnType } from '@/component/types/common';
import { GisStatType } from '@/component/types/stat';

interface SubFacPoiProps {
  area: ZnType;
  top: string;
  left: string;
  nm: string;
...
const SubFacPoi = ({
  ...
  const bgImage = STAT_POI_FAC_COLOR_GRADE[grade];

  return (
    <StyledSubFacPoi $top={top} $left={left} $bgImage={bgImage}>
...
const StyledSubFacPoi = styled.div<StyledSubPoiProps>`
  position: absolute;
  top: ${({ $top }) => $top};
  left: ${({ $left }) => $left};
  width: 68px;
