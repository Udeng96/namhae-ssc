...
const FacScChart = ({ scList, isAreaSelected }: { scList: EventStatSc[]; isAreaSelected: boolean }) => {
  ...
  useEffect(() => {
    if (scList.length > 0) {
      ...
      setMax(scList.length > limit ? limit : scList.length);
      setSeries(scList.map((sc) => sc.count));
      setCategories(scList.map((sc) => sc.scNm.replace('경로당', '')));
    } else {
      setSeries([0]);
      setCategories(['']);
    }
  }, [scList, isAreaSelected]);
