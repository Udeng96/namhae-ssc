...
const OperDateChart = ({ dateList }: { dateList: EventStatDate[] }) => {
  ...
  useEffect(() => {
    if (dateList.length === 0) {
      ...
      setSeries(dateList.map((item) => item.count['가동률']));
    }
  }, [dateList]);
  ...
  const options = {
    ...
    xAxis: {
      ...
      max, min: 0, scrollbar: { enabled: true },
