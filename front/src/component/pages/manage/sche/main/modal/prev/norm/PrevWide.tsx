...
const PrevWide = () => {
  const { titleOpt, selectContent, selectWideBack } = useScheStore(
    useShallow((s) => ({
      titleOpt:      s.titleOpt,
  ...
  const backUrl = selectWideBack
    ? `${BASE_URL}/newfile/${selectWideBack.fileNm}/${selectWideBack.fileType.split('/')[1]}/${SCHE_RESULT_TYPE.NORM}`
    : null;
...
const StyledBackDefault = styled.div`
  width: 100%;
  height: 100%;
  background: url("${SCHE_IMAGE.MAIN.BACK.NONE.WIDE}") no-repeat center / 100%;
`;
