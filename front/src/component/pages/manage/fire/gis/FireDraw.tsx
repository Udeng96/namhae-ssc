...
/**
 * 소방 전용 그리기 컴포넌트
 * - EventDraw.tsx와 동일한 기능이지만 fireGisStore.eventDrawType을 직접 구독
 */
const FireDraw = () => {
  const map           = useMap();
  const eventDrawType = useFireGisStore((s) => s.eventDrawType);
  ...
  useEffect(() => {
    ...
    if (distanceTooltip) {
      ...
      L.tooltip({
        permanent: false, interactive: true, offset: [0, 10],
        direction: 'bottom', className: 'distance-custom-tooltip',
      })
  ...
  useEffect(() => {
    ...
    if (areaTooltip) {
      ...
      L.tooltip({
        permanent: false, interactive: true, offset: [0, -30],
        direction: 'bottom', className: 'area-custom-tooltip',
      })
  ...
  useEffect(() => {
    if (linePositions.length > 0) {
      setPolylineEl(
        <Polyline positions={linePositions} color="#543FAF" weight={4} opacity={1} ref={polylineRef} />,
      );
    } else {
      setPolylineEl(null);
    }
  ...
  return (
    <>
      {activeType === DRAW_GIS_TYPE.DISTANCE
        ? polylineEl
        : activeType === DRAW_GIS_TYPE.EXTENT
          ? polygonEl
          : null}
    </>
  );
