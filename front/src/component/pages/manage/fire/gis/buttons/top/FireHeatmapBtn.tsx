...
/**
 * 소방 GIS 히트맵 버튼
 * - EventHeatmapBtn.tsx와 동일한 기능이지만 fireGisStore 사용
 */
const FireHeatmapBtn = () => {
  const { eventHeatmapOn, setEventHeatmapOn, setEventHeatmap } = useFireGisStore(
    useShallow((s) => ({
  ...
  // 드롭다운 열림 상태이거나 기간·타입이 바뀌면 히트맵 재조회
  useEffect(() => {
    if (!isOpen) return;
    ...
    (async () => {
      try {
        const res = await fetchEventHeatmap({
          ...
          endDtm:     date.endDtm,
          statEvetCd: heatmapType,
        });
        if (cancelled) return;
        setEventHeatmap(res.data ?? []);
        setEventHeatmapOn(true);
      } catch {
        if (cancelled) return;
        setEventHeatmap([]);
  ...
  return (
    <StyledHeatmapBtnBox>
      <StyledHeatmapBtn
        $isActive={isOpen || eventHeatmapOn}
