...
/**
 * 소방 GIS 타일 전환 버튼 (일반 / 위성)
 */
const FireTileBtn = () => {
  const { eventMapType, setEventMapType } = useFireGisStore(
    useShallow((s) => ({
  ...
  const handleBtn = (type: MapTileType) => {
    setEventMapType(type);
  };

  return (
    <StyledTileBtnBox>
      <StyledTileBtn
        $isActive={eventMapType === MAP_TILE_TYPE.NORMAL}
        onClick={() => handleBtn(MAP_TILE_TYPE.NORMAL)}
      >
        일반
      </StyledTileBtn>
      <StyledTileBtn
        $isActive={eventMapType === MAP_TILE_TYPE.SATELLITE}
