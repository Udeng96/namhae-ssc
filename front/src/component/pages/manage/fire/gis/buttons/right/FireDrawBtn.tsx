...
/**
 * 소방 GIS 그리기 버튼 (거리 / 면적 / 반경)
 */
const FireDrawBtn = () => {
  const { eventDrawType, eventRadius, setEventDrawType, setEventRadius } = useFireGisStore(
    useShallow((s) => ({
...
const StyledRadiusBtn = styled.button<{ $isActive: boolean }>`
  ...
  border: 1px solid
    ${({ $isActive }) => ($isActive ? 'rgba(243, 239, 255, 0.80)' : 'rgba(167, 174, 207, 0.50)')};
  border-radius: 50%;
  font-weight: 500;
  font-size: 12px;
  color: #f2f4fc;
