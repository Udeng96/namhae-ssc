...
import FireLastSearchDate from './FireLastSearchDate';
import FireLastSearchBox from './FireLastSearchBox';
...
const FireLastSearchArea = ({ onSearch, isFetching }: Props) => {
  const { lastFilter, setLastParam, setLastSelectEvent, setToastKey } = useFireStore(
    useShallow((state) => ({
  ...
  return (
    <>
      <StyledFields>
        <FireLastSearchDate />
        <FireLastSearchBox type="lastArea"  nm="경로당 지역" />
        <FireLastSearchBox type="lastEvent" nm="이벤트 유형" />
      </StyledFields>
...
export default FireLastSearchArea;
