...
import FireLastSearchArea from './search/FireLastSearchArea';
import FireLastEventResult from './FireLastEventResult';
...
const FireLastEventBody = () => {
  ...
  const { data, isFetching, isError, refetch } = useQuery({
    queryKey: ['fire', 'events', 'last'],
    // useFireStore.getState() 로 항상 최신 값 참조
    // → 페이지 클릭 직후 refetch() 해도 stale closure 문제 없음
    queryFn: () => {
      const { lastFilter: lf, lastParam: lp } = useFireStore.getState();
      return fetchFireEventList({
        znCd:       optsToParam(lf.areaOpts),
        statEvetCd: optsToParam(lf.typeOpts),
        startDtm:   lp.startDtm,
        endDtm:     lp.endDtm,
        pageNumber: lp.pageNumber - 1,  // store는 1-based, API는 0-based
        plcId:      '',
      });
    },
    enabled: false,
  ...
  return (
    <StyledBody>
      <StyledSearch>
        <FireLastSearchArea onSearch={handleSearch} isFetching={isFetching} />
      </StyledSearch>
      <FireLastEventResult
...
export default FireLastEventBody;
