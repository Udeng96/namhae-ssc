...
import FireLastAreaCheck from './check/FireLastAreaCheck';
import FireLastEventCheck from './check/FireLastEventCheck';
...
const FireLastSearchOption = ({ type }: Props) => {
  const { lastFilter, filterOpt, setFilterOpt } = useFireStore(
    useShallow((state) => ({
  ...
  const removeToken = (token: string) => {
    ...
    useFireStore.getState().actions.setLastFilter({ [key]: next });
  };
  ...
  return (
    <StyledOption>
      ...
      <StyledSelectBox $isOpen={isOpen}>
        <StyledSelectScroll>
          {type === 'lastArea'  && <FireLastAreaCheck />}
          {type === 'lastEvent' && <FireLastEventCheck />}
        </StyledSelectScroll>
        <StyledClsBtnWrap>
...
export default FireLastSearchOption;
